...
"""
import base64, pathlib, re, textwrap
...
<style>
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .section-label {{ display: inline-block; background: #1a4d8f; color: #fff; padding: 2px 9px; border-radius: 10px; font-size: 8.5pt; font-weight: 700; white-space: nowrap; }}
  ...
  @media print {{
    body {{ background: #fff; }}
    .exhibit-banner, .evidence-table th, .section-label, .part-badge {{
...
<table class="meta-table">
  ...
  <tr><td>Purpose</td><td>Demonstrate that BioForge is a real, functional software system in active development — not merely a proposed plan — with working code, reproducible outputs, and a public version-controlled repository.</td></tr>
  <tr><td>Petition Section</td><td>Prong II — Well Positioned to Advance the Proposed Endeavor (§3.4: Concrete Forward-Looking Research Plan)</td></tr>
...
<table class="meta-table">
  ...
  <tr><td>Key Result</td><td>49.1% yield improvement (28.90 → 43.08 g/l citric acid) | ANN R² = 0.99883</td></tr>
...
<table class="evidence-table">
  ...
  <tbody>
    ...
    <tr><td>Hollow-Fiber Separation Module</td><td style="color:#e65100;font-weight:700">In Progress</td><td>Yang-Cussler model for product recovery feasibility</td></tr>
    <tr><td>Process Simulation Engine</td><td style="color:#546e7a">Planned (Stage 2)</td><td>Mass/energy balances, heat integration</td></tr>
    <tr><td>Scale-Up Feasibility Assessment</td><td style="color:#546e7a">Planned (Stage 2)</td><td>Scale Readiness Score, pilot checklist</td></tr>
    <tr><td>Web Interface (FastAPI + frontend)</td><td style="color:#546e7a">Planned (Stage 2)</td><td>Any U.S. lab runs their waste stream via browser</td></tr>
  </tbody>
</table>
...
<div class="fig-container">
  ...
  <div class="fig-caption">Figure B-1 — BioForge Platform Architecture (7-layer block diagram). Generated by <code>docs/generate_architecture_diagram.py</code>. Published in the BioForge GitHub repository.</div>
</div>

<hr />

<!-- ══════════════════════════════════════════════════════════
     PART C — SOURCE CODE SUMMARY
════════════════════════════════════════════════════════════ -->
<div class="part-header">
  <span class="part-badge">Part C</span>
  <span class="part-title">Technical Stack &amp; Source Code</span>
</div>
...
<table class="evidence-table">
  <thead><tr><th>File</th><th>Contents</th><th>Lines</th></tr></thead>
  <tbody>
    <tr><td><span class="file-path">bioforge/optimization/doe_engine.py</span></td><td>Box-Behnken Design, Central Composite Design, Plackett-Burman screening — generates experimental matrices for any set of user-defined factors</td><td>~95</td></tr>
    <tr><td><span class="file-path">bioforge/optimization/ml_optimizer.py</span></td><td>ANN (MLPRegressor, tanh, MFFF), Random Forest (n=200), Gradient Boosting (lr=0.05), SVR (RBF, C=100) — trained with Leave-One-Out cross-validation; permutation importance sensitivity analysis; grid-search optimum finder</td><td>~187</td></tr>
    <tr><td><span class="file-path">bioforge/reporting/protocol_card.py</span></td><td>Generates structured, reproducible optimization protocol cards — includes optimal conditions, predicted yield, model metrics, factor sensitivity ranking, and reproducibility note linking to GitHub</td><td>~124</td></tr>
    <tr><td><span class="file-path">data/raw/citric_acid_doe_matrix.csv</span></td><td>All 17 experimental runs from published Box-Behnken Design (Okedi et al., 2024) — EDTA, Coconut Oil, Sodium Fluoride, Citric Acid yield</td><td>18 rows</td></tr>
    <tr><td><span class="file-path">notebooks/BioForge_CitricAcid_Optimization.ipynb</span></td><td>8-cell Jupyter notebook: load data → EDA → train models → validation → sensitivity → response surface → optimum → protocol card</td><td>~280</td></tr>
    <tr><td><span class="file-path">docs/generate_architecture_diagram.py</span></td><td>Matplotlib-based architecture block diagram generator</td><td>~160</td></tr>
    <tr><td><span class="file-path">requirements.txt</span></td><td>Python 3.11, NumPy 1.26, Pandas 2.2, Scikit-learn 1.3, SciPy 1.11, Matplotlib 3.8, Seaborn 0.13</td><td>7</td></tr>
  </tbody>
</table>
...
<!-- ══════════════════════════════════════════════════════════
     PART D / F — NOTEBOOK OUTPUT: FIGURES
════════════════════════════════════════════════════════════ -->
<div class="part-header">
  <span class="part-badge">Parts D &amp; F</span>
...
<h3>Figure 2 — Model Validation: Predicted vs. Actual &amp; Model Comparison</h3>
<div class="fig-container">
  <img src="data:image/png;base64,{b64(model)}" alt="Model Validation Plot" />
  <div class="fig-caption">Figure F-2 — <em>Left:</em> Predicted vs. experimental citric acid yield for the best-performing model. Points near the red diagonal indicate accurate predictions. <em>Right:</em> R² comparison across all four ML models (ANN, Random Forest, Gradient Boosting, SVR). Best model identified by Leave-One-Out cross-validation RMSE.</div>
</div>

<h3>Figure 3 — Factor Sensitivity Analysis</h3>
<div class="fig-container">
  <img src="data:image/png;base64,{b64(sens)}" alt="Sensitivity Analysis" />
  <div class="fig-caption">Figure F-3 — Permutation-based factor importance ranking. Sodium Fluoride accounts for &gt;63% of yield variance, consistent with published Sobol sensitivity indices (67.54%) from Okedi et al. (2024). This confirms that BioForge's ML models correctly identify the dominant process variable.</div>
</div>

<h3>Figure 4 — Response Surface: Yield Contour Map</h3>
<div class="fig-container">
  <img src="data:image/png;base64,{b64(surf)}" alt="Response Surface" />
  <div class="fig-caption">Figure F-4 — Predicted citric acid yield contour map as a function of Coconut Oil (%w/w) and Sodium Fluoride (g/l), with EDTA fixed at its optimal level (0.30 g/l). Experimental runs from the published Box-Behnken Design are overlaid as scatter points. The high-yield region (top-right) identifies optimal stimulant combinations.</div>
</div>
...
<div class="claim-row">
  ...
  <div class="evidence">Supported by: <span>Part E</span> — public GitHub repository initialized March 2026 with timestamped commits attributed solely to the petitioner; <span>Part A</span> — project overview explicitly documenting completed vs. in-progress modules.</div>
</div>
...
<div class="claim-row">
  ...
  <div class="evidence">Supported by: <span>Parts D &amp; F</span> — all four generated figures and the protocol card show BioForge running against the 17-run published Box-Behnken Design, reproducing the 49.1% yield improvement documented in the petition's citation of Okedi et al. (2024).</div>
</div>
...
<div class="claim-row">
  ...
  <div class="evidence">Supported by: <span>Part C</span> — <code>ml_optimizer.py</code> implements ANN (MLPRegressor, tanh, MFFF architecture), Random Forest, Gradient Boosting, and SVR; <code>doe_engine.py</code> implements Box-Behnken, Central Composite, and Plackett-Burman designs. <span>Part F</span> — Figure F-2 shows all four models trained and compared.</div>
</div>
...
<div class="claim-row">
  ...
  <div class="evidence">Supported by: <span>Part B</span> — architecture diagram shows the web interface, process simulation, and scale-up modules (Stage 2) require software engineering depth; <span>Part A</span> — roadmap aligns CS program completion (2029) with BioForge v2.0 web deployment milestone.</div>
</div>
...
<ul>
  ...
  <li><strong>Run the demonstration:</strong> Clone the repository, run <code>pip install -r requirements.txt</code>, then <code>python run_notebook.py</code> from the root directory. The notebook will reproduce all four figures and the protocol card above.</li>
...
<div class="callout">
  ...
  BioForge is documented as a prototype (v0.1) — this accurately reflects the early-career stage of Mr. Okedi's proposed endeavor. The petition does not claim BioForge is complete; it claims Mr. Okedi has begun building it and is well-positioned to complete it. This exhibit provides exactly that evidence: a real, working foundation proving the work has started, the methodology is sound, and the deliverables are achievable.
</div>
